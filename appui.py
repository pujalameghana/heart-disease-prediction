...
def show_auth_page():
    ...
    with col2:
        ...
        <div style='background:#161B22; border:1px solid #30363D; border-radius:14px;
            ...
            <p style='color:#8B949E; font-size:0.8rem; margin-top:0.3rem; margin-bottom:0;'>
                ModelOps Framework Dashboard
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with tab2:
            ...
            if st.button("📝 Create Account", key="signup_btn"):
                ...
                if not all([n, u, p, cp]):
                    ...
                    if success:
                        st.success("✅ Account created! Go to Sign In tab to login.")
                    else:
                        st.error(f"❌ {message}")
...
with st.sidebar:
    ...
    <div style='font-size:0.78rem; color:#8B949E; margin-bottom:1rem;'>
        Welcome, <b style='color:#58A6FF'>
        {st.session_state.get('name', '')}</b>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style='font-size:0.75rem; color:#8B949E; margin-top:1rem;'>
        <div style='margin-bottom:0.3rem;'>Date</div>
...
if page == "🏠 Dashboard":
    ...
    with c1:
        st.markdown("<div class='metric-card'><p class='metric-value'>86.97%</p><p class='metric-label'>Accuracy</p></div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div class='metric-card'><p class='metric-value' style='color:#3FB950'>94.50%</p><p class='metric-label'>ROC-AUC</p></div>", unsafe_allow_html=True)
    with c3:
        st.markdown("<div class='metric-card'><p class='metric-value' style='color:#D29922'>87.84%</p><p class='metric-label'>F1 Score</p></div>", unsafe_allow_html=True)
    with c4:
        st.markdown("<div class='metric-card'><p class='metric-value' style='color:#E63946'>1190</p><p class='metric-label'>Training Samples</p></div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        with r1c3: chest_pain = st.selectbox("Chest Pain Type", [0,1,2,3],
        ...
        with r2c3: fasting_bs = st.selectbox("Fasting Blood Sugar>120", [0,1],
                       format_func=lambda x: "No" if x==0 else "Yes")

        r3c1, r3c2, r3c3 = st.columns(3)
        with r3c1: resting_ecg = st.selectbox("Resting ECG", [0,1,2],
                       format_func=lambda x: ["Normal","ST-T Abnormality","LV Hypertrophy"][x])
        with r3c2: max_hr = st.slider("Max Heart Rate", 60, 220, 168)
        with r3c3: exercise_angina = st.selectbox("Exercise Angina", [0,1],
                       format_func=lambda x: "No" if x==0 else "Yes")

        r4c1, r4c2 = st.columns(2)
        with r4c1: oldpeak = st.slider("Oldpeak", 0.0, 6.0, 1.0, 0.1)
        with r4c2: st_slope = st.selectbox("ST Slope", [0,1,2],
            ...
            if not model_loaded:
                ...
                if pred == 1:
                    ...
                    <div class='result-positive'>
                        ...
                        <p style='color:{rcolor}; font-weight:600'>{risk} Risk</p>
                    </div>""", unsafe_allow_html=True)
                else:
                    st.markdown(f"""
...
# ══════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        <div style='background:#161B22;border:1px solid #30363D;border-radius:8px;
            ...
            <b>How it works:</b><br>
            1. Compares reference data with current incoming data<br>
    ...
    with col2:
        ...
        if "pipeline_output" in st.session_state:
            ...
            if "PIPELINE COMPLETE" in output:
                ...
                <div style='background:rgba(63,185,80,0.1);border:1px solid #3FB950;
                            border-radius:8px;padding:1rem;margin-top:1rem;text-align:center;'>
                    <b style='color:#3FB950'>✅ Pipeline Completed Successfully</b>
                </div>""", unsafe_allow_html=True)
